...
#          # 定义LSTM层
#         self.lstm = nn.LSTM(100, 256, num_layers=2, dropout=0.5, batch_first=True)
#         # 定义全连接层
...
class LSTM(nn.Module):
    def __init__(self,vocab_size,num_class):
        ...
        self.embedding = nn.Embedding(vocab_size, 100)
         # 定义LSTM层
        self.lstm = nn.LSTM(100, 256, num_layers=2, bidirectional=True, dropout=0.5, batch_first=True)
        # 定义全连接层
    ...
    def train(net,train_iter,val_iter,learning_rate,num_epochs,weight_decay,device):
        ...
        for epoch in range(num_epochs):
            ...
            if valid_loss < best_loss :
                ...
                torch.save(net.state_dict(), 'LSTM/LSTM_state_dict_y.pth')
                print('saving model with loss {:.3f}'.format(best_loss))
    def test(X_test,y_test,vocab_size,device):
        model = LSTM(vocab_size,2)

        model = model.to(device)
        model.load_state_dict(torch.load('LSTM/LSTM_state_dict_y.pth'))
...
if __name__ == "__main__":
    lr, num_epochs, batch_size, weight_decay= 3e-3, 10, 32,1e-3
    device = get_device()
    X_train,X_val,X_test,y_train,y_val,y_test,vocab_size = getData()
    # train_dataset = MyDataset(X_train, y_train)
    # val_dataset = MyDataset(X_val, y_val)
    # net = LSTM(vocab_size,2)
