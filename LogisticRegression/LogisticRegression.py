...
class LRegression(nn.Module):
    def __init__(self, input_size,vocab_size):
        ...
        self.embed = nn.Embedding(vocab_size, 64, padding_idx=1)
        self.flatten = nn.Flatten()
        self.linear = nn.Linear(input_size*64, 1)
    ...
    def train(net,train_iter,val_iter,learning_rate,num_epochs,weight_decay,device):
        ...
        for epoch in range(num_epochs):
            ...
            if valid_loss < best_loss :
                ...
                torch.save(net.state_dict(), 'LogisticRegression/LRegression_state_dict_y.pth')
                print('saving model with loss {:.3f}'.format(best_loss))
    def test(X_test,y_test,vocab_size,device):
        model = LRegression(X_test.shape[1],vocab_size)

        model = model.to(device)
        model.load_state_dict(torch.load('LogisticRegression/LRegression_state_dict_y.pth'))
...
if __name__ == "__main__":
    ...
    # net = LRegression(X_train.shape[1],vocab_size)
