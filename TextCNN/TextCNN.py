...
class TextCNN(nn.Module):
    def __init__(self,vocab_size,num_class):
        ...
        self.conv11 = nn.Conv2d(1, 16, (3, 64))
        self.conv12 = nn.Conv2d(1, 16, (4, 64))
        self.conv13 = nn.Conv2d(1, 16, (5, 64))
        self.dropout = nn.Dropout(0.5)
        self.fc1 = nn.Linear(3 * 16, num_class)
    ...
    def train(net,train_iter,val_iter,learning_rate,num_epochs,weight_decay,device):
        ...
        for epoch in range(num_epochs):
            ...
            if valid_loss < best_loss :
                ...
                torch.save(net.state_dict(), 'TextCNN/TextCNN_state_dict_y.pth')
                print('saving model with loss {:.3f}'.format(best_loss))
    def test(X_test,y_test,vocab_size,device):
        model = TextCNN(vocab_size,2)

        model = model.to(device)
        model.load_state_dict(torch.load('TextCNN/TextCNN_state_dict_y.pth'))
