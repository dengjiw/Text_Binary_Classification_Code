...
#创建一个模型
class DNN(nn.Module):
    def __init__(self,input_dim,vocab_size,num_class):
        super(DNN, self).__init__()
        self.embed = nn.Embedding(vocab_size, 64, padding_idx=1)
        self.flatten = nn.Flatten()
        self.fc1 = nn.Linear(input_dim * 64, 256)
    ...
    def train(net,train_iter,val_iter,learning_rate,num_epochs,weight_decay,device):
        ...
        for epoch in range(num_epochs):
            ...
            if valid_loss < best_loss :
                ...
                torch.save(net.state_dict(), 'DNN/DNN_state_dict_y.pth')
                print('saving model with loss {:.3f}'.format(best_loss))
    def test(X_test,y_test,vocab_size,device):
        model = DNN(X_test.shape[1],vocab_size,2)

        model = model.to(device)
        model.load_state_dict(torch.load('DNN/DNN_state_dict_y.pth'))
...
if __name__ == "__main__":
    ...
    # net = DNN(X_train.shape[1],vocab_size,2)
